...
def fetch_request_api(query_char):
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f'Error in API fetching, status {response.status_code}. Check the API and try again')
    return response
...
def main(args):
    for password in args:
        ...
        if count:
            print(
                f'{password} was found {count} times. You should probably use different password')
        else:
